//  Code for showing information about camera and camera parameters in log file
...
//	Class with static methods for dumping potentially useful information in the log
class ExtraInfo
{
	// put info about display into log
    static void showDisplayInfo (Context context) // need context
    {
    ...
    static void showPreviewSizes (Camera mCamera)
    {
    ...
    static void showCameraInfo (Camera mCamera)
    {
        Camera.Parameters params = mCamera.getParameters();
    ...
    static void showCameraInfoAll ()
    {
		boolean bShowFlattenFlag=true;	// show parameters.flatten()
		boolean bShowDumpFlag=false;	    // show parameters.dump()
        ...
        for (int k=0; k < numCam; k++) {
            ...
            Log.i(TAG, str);
			// don't do this when a camera is already open 
            if (MainActivity.mCamera == null) {	// sanity check
				Camera mCamera = MainActivity.openCamera(k);
				showCameraInfo(mCamera);	// go show some details
				if (bShowFlattenFlag)
					showCameraFlatten(mCamera); // full set of parameters
				if (bShowDumpFlag)
					showCameraDump(mCamera); // full set of parameters
				MainActivity.releaseCamera(mCamera);
			}
        }
    }

	// convert code to human readable form
    private static String pictureFormatString (int iform)
    {
        switch(iform) {
   ...
   // show full set of parameters for camera using "flatten" method in Camera.Parameters
    private static void showCameraFlatten (Camera mCamera)
    {
    ...
    private static void showCameraDump (Camera mCamera)
    {
		boolean DBG=false;
...
// NOTE:  flatten and dump methods appear to produce the same information.
